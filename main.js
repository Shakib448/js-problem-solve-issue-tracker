...
const closeIssue = id => {
  ...
  // const currentIssue = issues.find(issue => issue.id === id); main (=== this will check matches also the type of the value)
...
const deleteIssue = id => {
  const issues = JSON.parse(localStorage.getItem('issues'));
  // const remainingIssues = issues.filter( issue.id !== id ) main (!== this will check not  matching also the type of the value)
  ...
  // localStorage.setItem('issues', JSON.stringify(remainingIssues)); this data is intergrade if estetment 
  // FOr delete local storage data
...
// Why this methods are use inside the for loop because of this issuesList.innerHTML are inner in for loop thats why the algorithm works like
   ...
   // This ( ? ) contains the if estetment and the ( : ) this contains the else estetment
